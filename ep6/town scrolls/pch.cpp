...
void __declspec(naked) packet_hook() {
	__asm {
		...
		*/
		...
		mov byte ptr[ecx+0x5930],dl
		mov dl,byte ptr[ebp+0x3]
		mov byte ptr[ecx+0x5934],dl
		mov dl,byte ptr[ebp+0x4]
		mov byte ptr[ecx+0x5938],dl

		mov edi,ecx //user = ecx
...
void __declspec(naked) effect_hook() {
	__asm {
		...
		cmp byte ptr[ebp+0x5938],0x0
		je _beika
		cmp byte ptr[ebp+0x5938],0x1
		je _aelbeageu
		cmp byte ptr[ebp+0x5938],0x2
		je _keolloseu
		jmp effect_fail
